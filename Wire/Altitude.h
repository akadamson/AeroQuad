...
public: 
  ...
  void initialize(void) {     
    updateWordRegisterI2C(ALT_ADDRESS, 0x01, 0xC2, 0xE3); // Conf. reg: AIN0 single ended, 1x gain, cont. sampl., 860SPS, comp. disable              
    sendByteI2C(ALT_ADDRESS, 0x00); // Pointer register points to Conversion register, set to go
  ...
  void measure(void) {
    rawADC = readWordI2C(ALT_ADDRESS); // read two bytes from 7-bit address 0x48 followed by high R/W bit
...
//  int altitudeAddress;
  ...
  void requestRawPressure(void) {
    updateRegisterI2C(ALT_ADDRESS, 0xF4, 0x34+(overSamplingSetting<<6));
  }
  
  long readRawPressure(void) {
    unsigned char msb, lsb, xlsb;
    sendByteI2C(ALT_ADDRESS, 0xF6);
    Wire.requestFrom(ALT_ADDRESS, 3); // request three bytes
  ...
  void requestRawTemperature(void) {
    updateRegisterI2C(ALT_ADDRESS, 0xF4, 0x2E);
  }
  
  unsigned int readRegister(byte r) {
    sendByteI2C(ALT_ADDRESS, r);
    return readWordWaitI2C(ALT_ADDRESS);
  }
