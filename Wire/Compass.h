...
public: 
  ...
  void initialize(void) {
    ...
    while (success == false && numAttempts < 5 ) {
      ...
      updateRegisterI2C(COMP_ADDRESS, 0x00, 0x11);  // Set positive bias configuration for sensor calibraiton
      delay(50);
   
      updateRegisterI2C(COMP_ADDRESS, 0x01, 0x20); // Set +/- 1G gain
      delay(10);

      updateRegisterI2C(COMP_ADDRESS, 0x02, 0x01);  // Perform single conversion
      delay(10);
      ...
      updateRegisterI2C(COMP_ADDRESS, 0x00, 0x10);  // Set 10hz update rate and normal operaiton
      delay(50);

      updateRegisterI2C(COMP_ADDRESS, 0x02, 0x00); // Continuous Update mode
  ...
  void measure(float roll, float pitch) {
    ...
    sendByteI2C(COMP_ADDRESS, 0x03);
    Wire.requestFrom(COMP_ADDRESS, 6);
